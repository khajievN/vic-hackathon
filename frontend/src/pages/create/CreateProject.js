...
import { Box, Button, TextField, Typography, InputLabel, Input, FormControl, Modal, Stepper, Step, StepLabel } from '@mui/material';
import ReactQuill from 'react-quill';
import 'react-quill/dist/quill.snow.css';
import { createProjectMetadataApi } from "../../hooks/useAPI";
...
const CreateProject = () => {
    ...
    const { checkAllowanceForCreateProject, approveForCreateProject, createProject } = useWeb3();
    ...
    const handleSubmit = async () => {
        try {
            ...
            formData.append('grant_amount', grantPrice);
            if (file) {
                formData.append('file', file);
            }
            const res = await createProjectMetadataApi(formData);
            console.log("res", res.data);
            const metadata = res.data.data.metadata;
            setActiveStep(1); // Move to next step

            // Step 2: Approving
            const allowance = await checkAllowanceForCreateProject();
            console.log("allowance", allowance);
            if (Number.parseFloat(allowance) <= 0) {
                const approveRes = await approveForCreateProject();
                console.log('approveRes', approveRes);
            }
            setActiveStep(2); // Move to next step

            // Step 3: Creating project
            const createProjectRes = await createProject(metadata, grantPrice);
